...
class UR_network_build:
    ...
    def create_mask(self,reservoir_size,input_size,output_size,reservoir_connection_density=0.1,input_output_connection_density=0.1):
    ...
    def build_one_network_w_IE_neurons_v1(self, net_num, num_neurons_in_res=1000,
        ...
        mask = self.create_mask(reservoir_size=num_neurons_in_res, input_size=num_input_neurons,
                              ...
                              input_output_connection_density=input_output_conn_density)

        weights = self.create_weight_matrix(num_neurons_in_res, num_input_neurons, num_output_neurons,
...
def main(argv):
    ...
    log_average_ranges = [-2.3,-1.5]
    log_var_ranges = [0.15, 1.2]

    initialization_not_satisfied = True
    while initialization_not_satisfied:
        log_ave_w = log_average_ranges[0] + np.absolute((log_average_ranges[1] - log_average_ranges[0])) * np.random.random()
        log_var_w = log_var_ranges[0] + np.absolute((log_var_ranges[1] - log_var_ranges[0])) * np.random.random()
